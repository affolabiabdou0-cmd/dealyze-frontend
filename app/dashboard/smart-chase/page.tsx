...
import { Mail, Copy, Check, AlertCircle, Sparkles, Calendar, Download } from "lucide-react";
...
export default function SmartChasePage() {
  ...
  function downloadPDF() {
    ...
    if (!win) return;
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Relance — ${result.client_name}</title>
    <style>body{font-family:system-ui,sans-serif;max-width:700px;margin:40px auto;color:#111;line-height:1.7;padding:0 20px}
    h1{font-size:20px;font-weight:700}h2{font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#f97316;margin:20px 0 8px}
    .meta{color:#666;font-size:13px;margin-bottom:24px}.subject{background:#f4f4f4;padding:12px 16px;border-radius:8px;font-weight:600;margin-bottom:16px}
    pre{white-space:pre-wrap;font-family:inherit;margin:0}@media print{body{margin:20px}}</style></head>
    <body><h1>Relance — ${result.client_name}</h1>
    <p class="meta">${result.chase_id} · ${result.amount_display} · ${result.days_overdue} jours de retard</p>
    <h2>Objet</h2><div class="subject">${result.email_subject}</div>
    <h2>Corps du mail</h2><pre>${result.email_body}</pre></body></html>`);
    win.print(); win.close();
  }
              ...
              <Card style={{ animation: "fadeInSection 0.4s ease both" }}>
                ...
                {result.next_action_date && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", borderRadius: 8, background: "#0f0f13", border: "1px solid #2a2a3a" }}>
                    <Calendar size={13} style={{ color: COLOR, flexShrink: 0 }} strokeWidth={1.5} />
                    <span style={{ fontSize: 12, color: "#6a6a8a" }}>Prochaine action : <strong style={{ color: "#94a3b8" }}>{result.next_action_date}</strong></span>
                  </div>
                )}
              </Card>
              <Card style={{ animation: "fadeInSection 0.4s ease both", animationDelay: "120ms" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <Mail size={14} style={{ color: COLOR }} strokeWidth={1.5} />
                    <span style={{ fontSize: 13, fontWeight: 600, color: "#94a3b8" }}>Email de relance</span>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={copyEmail} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 7, border: "1px solid #2a2a3a", background: "none", cursor: "pointer", color: "#6a6a8a", fontSize: 11 }}>
