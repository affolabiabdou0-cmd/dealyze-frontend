...
import { Shield, Copy, Check, AlertCircle, Sparkles, Download, User, Building2, TrendingUp, AlertTriangle, CheckCircle2, XCircle, FileSearch } from "lucide-react";
...
export default function DeepDuePage() {
  ...
  function downloadPDF() {
    ...
    const risksHtml = result.risques_identifies.map((r) => {
      ...
      return `<div class="risk-item" style="border-left:3px solid ${rs.color}">
        <span class="risk-badge" style="background:${rs.bg};color:${rs.color}">${rs.label}</span>
        <p class="risk-desc">${r.description}</p>
      </div>`;
    }).join("");

    win.document.write(`<!DOCTYPE html>
<html lang="fr"><head><meta charset="utf-8"><title>Due Diligence — ${result.company_name}</title>
<style>
...
.subtitle{font-size:14px;color:#059669;font-weight:600}
.hero{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:32px}
.hero-card{padding:20px;border-radius:12px;border:1px solid #e2e8f0}
...
.signal-box.pos{background:#f0fdf4;border:1px solid #86efac}
.signal-box.neg{background:#fef2f2;border:1px solid #fca5a5}
...
.footer-badge{background:#059669;color:#fff;font-size:11px;font-weight:700;padding:6px 16px;border-radius:8px}
@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.page{padding:24px 32px}}
</style></head><body>
<div class="page">
  <div class="header">
    <div class="logo">Deep<span>Due</span> <span style="font-size:13px;color:#9090b0;font-weight:400">by Dealyze</span></div>
    <div class="meta-right">
      <div class="due-id">${result.due_id}</div>
      <div class="due-date">Analysé le ${new Date(result.generated_at).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</div>
    </div>
  </div>
  ...
  <div class="hero">
    ...
    <div class="hero-card">
      ...
      <div class="conf-num">${result.score_confiance}<span style="font-size:14px;color:#9090b0">/10</span></div>
      <div class="conf-bar-bg"><div class="conf-bar-fill"></div></div>
      <div style="font-size:11px;color:#9090b0">Qualité et quantité des données disponibles</div>
    </div>
  </div>
  ...
  <div class="section-title">Profil fondateur</div>
  ...
  ${result.profil_fondateur.reputation ? `<p class="prose"><strong>Réputation :</strong> ${result.profil_fondateur.reputation}</p>` : ""}
  ${(result.profil_fondateur.signaux_positifs?.length || result.profil_fondateur.signaux_negatifs?.length) ? `
  <div class="two-col" style="margin-top:12px">
    <div class="signal-box pos">
      <div class="signal-title">Signaux positifs</div>
      <ul>${(result.profil_fondateur.signaux_positifs ?? []).map((s) => `<li>${s}</li>`).join("")}</ul>
    </div>
    <div class="signal-box neg">
      <div class="signal-title">Signaux négatifs</div>
      <ul>${(result.profil_fondateur.signaux_negatifs ?? []).map((s) => `<li>${s}</li>`).join("")}</ul>
    </div>
  </div>` : ""}` : ""}

  ${result.analyse_entreprise ? `
  <div class="section-title">Analyse entreprise</div>
  ...
  <div class="footer">
    <div class="footer-left">Document généré par Dealyze Deep Due IA · ${new Date().getFullYear()}<br>Rapport confidentiel — Usage investisseur uniquement</div>
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: "#4a4a6a" }}>Due diligence automatisée · Profil fondateur · Cartographie des risques</p>
        </div>
      </div>
      ...
      <div className={result ? "block" : "grid lg:grid-cols-2 gap-5"}>
        ...
        {!result && !loading && (
          <div style={{ background: "#1a1a24", border: "1px solid #2a2a3a", borderRadius: 16, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 320, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: "#4a4a6a", maxWidth: 250, lineHeight: 1.6 }}>Entrez le nom d&apos;une entreprise pour lancer une due diligence complète avec cartographie des risques.</p>
          </div>
        )}
          ...
          <div style={{ animation: "fadeInFast 0.4s ease both" }}>
            ...
            <div style={{ background: "#1a1a24", border: "1px solid #2a2a3a", borderRadius: 20, overflow: "hidden" }}>
              ...
              <div style={{ background: `linear-gradient(135deg,${COLOR}18,#1a1a24)`, borderBottom: "1px solid #2a2a3a", padding: "28px 32px", animation: "fadeInSection 0.4s ease both" }}>
                <div style={{ fontFamily: "monospace", fontSize: 11, color: "#3a3a5a", marginBottom: 6 }}>{result.due_id}</div>
                <h3 style={{ fontSize: 22, fontWeight: 800, color: "#f1f5f9", letterSpacing: "-0.5px", marginBottom: 4 }}>Due Diligence — {result.company_name}</h3>
                {result.founder_name && <p style={{ fontSize: 13, color: "#4a4a6a", marginBottom: 16 }}>Fondateur : {result.founder_name}</p>}
                {/* Summary cards */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  ...
                  <div style={{ padding: "14px 16px", borderRadius: 12, background: "#0f0f13", border: "1px solid #2a2a3a" }}>
                    <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#4a4a6a", marginBottom: 8 }}>Score de confiance</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontFamily: "monospace", fontSize: 14, fontWeight: 800, color: "#f1f5f9" }}>{result.score_confiance}/10</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div style={{ padding: "24px 32px", borderBottom: "1px solid #1e1e2e", animation: "fadeInSection 0.4s ease both", animationDelay: "80ms" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: `${COLOR}15`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <FileSearch size={13} style={{ color: COLOR }} strokeWidth={1.5} />
                    </div>
                    <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: COLOR }}>Synthèse exécutive</span>
                  </div>
                  <p style={{ fontSize: 13.5, color: "#94a3b8", lineHeight: 1.75, paddingLeft: 36 }}>{result.synthese_executive}</p>
                </div>
              )}
              ...
              {result.profil_fondateur && (
                <div style={{ padding: "24px 32px", borderBottom: "1px solid #1e1e2e", animation: "fadeInSection 0.4s ease both", animationDelay: "160ms" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: `${COLOR}15`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <User size={13} style={{ color: COLOR }} strokeWidth={1.5} />
                    </div>
                  ...
                  <div style={{ paddingLeft: 36 }}>
                    ...
                    {result.profil_fondateur.experience && (
                      <div style={{ marginBottom: 10 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: "#4a4a6a", marginBottom: 4 }}>Expérience</div>
                    ...
                    {(result.profil_fondateur.signaux_positifs?.length > 0 || result.profil_fondateur.signaux_negatifs?.length > 0) && (
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
                        {result.profil_fondateur.signaux_positifs?.length > 0 && (
                          <div style={{ padding: "12px 14px", borderRadius: 10, background: "#0f2318", border: "1px solid #10b98133" }}>
                            ...
                            {result.profil_fondateur.signaux_positifs.map((s, i) => (
                              <div key={i} style={{ fontSize: 12, color: "#94a3b8", display: "flex", gap: 6, marginBottom: 4 }}><span style={{ color: "#10b981", flexShrink: 0 }}>+</span>{s}</div>
                            ))}
                          </div>
                        )}
                        {result.profil_fondateur.signaux_negatifs?.length > 0 && (
                          <div style={{ padding: "12px 14px", borderRadius: 10, background: "#1f1015", border: "1px solid #ef444433" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                            ...
                            {result.profil_fondateur.signaux_negatifs.map((s, i) => (
                              <div key={i} style={{ fontSize: 12, color: "#94a3b8", display: "flex", gap: 6, marginBottom: 4 }}><span style={{ color: "#ef4444", flexShrink: 0 }}>−</span>{s}</div>
                            ))}
                          </div>
                        )}
                      </div>
              ...
              {result.analyse_entreprise && (
                <div style={{ padding: "24px 32px", borderBottom: "1px solid #1e1e2e", animation: "fadeInSection 0.4s ease both", animationDelay: "240ms" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: `${COLOR}15`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Building2 size={13} style={{ color: COLOR }} strokeWidth={1.5} />
                    </div>
                    <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: COLOR }}>Analyse entreprise</span>
                  </div>
                  <div style={{ paddingLeft: 36 }}>
                    {result.analyse_entreprise.resume && <p style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.7, marginBottom: 10 }}>{result.analyse_entreprise.resume}</p>}
                    {result.analyse_entreprise.position_marche && (
                      <div style={{ marginBottom: 12 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: "#4a4a6a", marginBottom: 4 }}>Position marché</div>
                        <p style={{ fontSize: 13, color: "#94a3b8" }}>{result.analyse_entreprise.position_marche}</p>
                      </div>
                    )}
                    {result.analyse_entreprise.concurrents?.length > 0 && (
                      <div style={{ marginBottom: 12 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: "#4a4a6a", marginBottom: 8 }}>Concurrents</div>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {result.analyse_entreprise.concurrents.map((c, i) => (
                            <span key={i} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 20, background: "#0f0f13", border: "1px solid #2a2a3a", color: "#94a3b8" }}>{c}</span>
                          ))}
                        </div>
                      </div>
                    )}
                ...
                <div style={{ padding: "24px 32px", borderBottom: "1px solid #1e1e2e", animation: "fadeInSection 0.4s ease both", animationDelay: "320ms" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: "#ef444415", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <AlertTriangle size={13} style={{ color: "#ef4444" }} strokeWidth={1.5} />
                    </div>
                  ...
                  <div style={{ paddingLeft: 36, display: "flex", flexDirection: "column", gap: 8 }}>
                    {result.risques_identifies.map((risk, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px", borderRadius: 10, background: "#0f0f13", borderLeft: `3px solid ${rs.color}`, border: `1px solid ${rs.color}22` }}>
                          <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 5, background: rs.bg, color: rs.color, flexShrink: 0, marginTop: 2 }}>{rs.label}</span>
                          <p style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.6 }}>{risk.description}</p>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div style={{ padding: "20px 32px", background: "#0f0f13", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <p style={{ fontSize: 11, color: "#3a3a5a" }}>Rapport généré par Dealyze Deep Due IA · XPRIZE 2026</p>
