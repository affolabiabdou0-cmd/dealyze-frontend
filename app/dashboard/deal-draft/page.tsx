...
export default function DealDraftPage() {
  ...
  function buildText() {
    ...
    if (Array.isArray(c.livrables) && c.livrables.length) {
  ...
  function downloadPDF() {
    ...
    const livrablesHtml = Array.isArray(c.livrables) && c.livrables.length
      ? `<div class="section">
          <div class="section-label">Livrables inclus</div>
          <ul>${(c.livrables as string[]).map((l) => `<li>${l}</li>`).join("")}</ul>
        </div>` : "";
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .quote-date { font-size: 13px; color: #555; }
...
<div class="page">
  <div class="header">
    <div class="logo">Deal<span>yze</span></div>
    <div class="quote-meta">
      <div class="quote-id">${result.quote_id}</div>
      <div class="quote-date">Généré le ${new Date(result.generated_at).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</div>
    </div>
  </div>
  ...
  <div class="client-bar">
    <div class="client-item">
      <div class="client-item-label">Client</div>
      <div class="client-item-value">${result.client_name}</div>
    </div>
    <div class="client-item">
      <div class="client-item-label">Secteur</div>
      <div class="client-item-value">${form.sector || "—"}</div>
    </div>
    <div class="client-item">
      <div class="client-item-label">Budget</div>
      <div class="client-item-value">${form.budget || "—"}</div>
    </div>
    <div class="client-item">
      <div class="client-item-label">Délai</div>
      <div class="client-item-value">${form.timeline || "—"}</div>
    </div>
  </div>
  ...
  <div class="footer">
    <div class="footer-left">
      Document généré par Dealyze AI · ${new Date().getFullYear()}<br>
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: "#4a4a6a" }}>Devis professionnel généré par IA en quelques secondes</p>
        </div>
      </div>
      ...
      <div className={result ? "block" : "grid lg:grid-cols-2 gap-5"}>
          ...
          <div style={{ background: "#1a1a24", border: "1px solid #2a2a3a", borderRadius: 16, padding: 24 }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <button type="submit" disabled={loading}
                style={{ width: "100%", padding: "14px 20px", borderRadius: 10, border: "none", background: `linear-gradient(135deg, ${COLOR}, #6d28d9)`, color: "#fff", fontSize: 14, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, cursor: loading ? "default" : "pointer", opacity: loading ? 0.7 : 1, boxShadow: `0 4px 14px ${GLOW}`, transition: "box-shadow 0.15s" }}
              ...
              >
                {loading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Génération…</> : <><Sparkles size={15} strokeWidth={1.5} /> Générer le devis</>}
              </button>
            </form>
          </div>
        )}
        ...
        {loading && !result && (
          <div style={{ background: "#1a1a24", border: "1px solid #2a2a3a", borderRadius: 16, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 320 }}>
            ...
            <p style={{ fontSize: 13, color: "#4a4a6a" }}>Gemini génère votre devis…</p>
          </div>
        )}
        ...
        {!result && !loading && (
          <div style={{ background: "#1a1a24", border: "1px solid #2a2a3a", borderRadius: 16, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 320, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: "#4a4a6a", maxWidth: 220, lineHeight: 1.6 }}>Remplissez le formulaire et cliquez sur Générer pour obtenir votre devis instantanément.</p>
          </div>
        )}

        {/* ── RESULT — full professional quote ── */}
        {result && (
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={copyText}
                  style={{ display: "flex", alignItems: "center", gap: 5, padding: "8px 14px", borderRadius: 8, border: "1px solid #2a2a3a", background: "none", cursor: "pointer", color: "#6a6a8a", fontSize: 12 }}>
                  {copied ? <><Check size={12} style={{ color: "#10b981" }} /> Copié</> : <><Copy size={12} strokeWidth={1.5} /> Copier</>}
                </button>
                <button onClick={downloadPDF}
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, border: "none", background: `linear-gradient(135deg, ${COLOR}, #6d28d9)`, cursor: "pointer", color: "#fff", fontSize: 12, fontWeight: 700, boxShadow: `0 2px 8px ${GLOW}` }}>
            ...
            <div style={{ background: "#1a1a24", border: "1px solid #2a2a3a", borderRadius: 20, overflow: "hidden" }}>
              ...
              <div style={{ background: `linear-gradient(135deg, ${COLOR}22, #1a1a24)`, borderBottom: "1px solid #2a2a3a", padding: "28px 32px", animation: "fadeInSection 0.4s ease both" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 11, fontFamily: "monospace", color: "#3a3a5a", marginBottom: 6, letterSpacing: "0.05em" }}>{result.quote_id}</div>
                    <h3 style={{ fontSize: 22, fontWeight: 800, color: "#f1f5f9", letterSpacing: "-0.5px", lineHeight: 1.2, marginBottom: 6 }}>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 11, color: "#3a3a5a", marginBottom: 4 }}>
                      {new Date(result.generated_at).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}
                    </div>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 20, background: `${COLOR}20`, color: COLOR }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                  ...
                  ].filter((p) => p.label).map(({ Icon, label }, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", borderRadius: 20, background: "#0f0f13", border: "1px solid #2a2a3a", fontSize: 12, color: "#94a3b8" }}>
              ...
              <div style={{ padding: "8px 0" }}>
                {CONTENT_SECTIONS.map((s, i) => {
                  ...
                  return (
                    <div key={s.key}
                      style={{ padding: "22px 32px", borderBottom: i < CONTENT_SECTIONS.length - 1 ? "1px solid #1e1e2e" : "none", animation: "fadeInSection 0.4s ease both", animationDelay: `${i * 70}ms` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                        <div style={{ width: 28, height: 28, borderRadius: 8, background: `${COLOR}15`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          <s.Icon size={13} style={{ color: COLOR }} strokeWidth={1.5} />
                        </div>
                        <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: COLOR }}>{s.label}</span>
                      </div>
                      <p style={{ fontSize: 13.5, color: "#94a3b8", lineHeight: 1.75, paddingLeft: 36 }}>{val}</p>
                    </div>
                  );
                })}
                ...
                {Array.isArray(result.content.livrables) && result.content.livrables.length > 0 && (
                  <div style={{ padding: "22px 32px", borderTop: "1px solid #1e1e2e", animation: "fadeInSection 0.4s ease both", animationDelay: "560ms" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 8, background: `${COLOR}15`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <Package size={13} style={{ color: COLOR }} strokeWidth={1.5} />
                      </div>
                    ...
                    <div style={{ paddingLeft: 36, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      {(result.content.livrables as string[]).map((l, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                          <div style={{ width: 18, height: 18, borderRadius: 5, background: `${COLOR}20`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
              ...
              {/* Footer */}
              <div style={{ padding: "20px 32px", borderTop: "1px solid #2a2a3a", background: "#0f0f13", display: "flex", alignItems: "center", justifyContent: "space-between", animation: "fadeInSection 0.4s ease both", animationDelay: "640ms" }}>
                <div>
                  <p style={{ fontSize: 11, color: "#3a3a5a", lineHeight: 1.6 }}>Devis valable 30 jours · Généré par Dealyze AI</p>
                  <p style={{ fontSize: 11, color: "#3a3a5a" }}>XPRIZE AI Hackathon 2026</p>
                </div>
                <button onClick={downloadPDF}
                  style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 20px", borderRadius: 10, border: "none", background: `linear-gradient(135deg, ${COLOR}, #6d28d9)`, cursor: "pointer", color: "#fff", fontSize: 13, fontWeight: 700, boxShadow: `0 4px 14px ${GLOW}` }}>
                  <Download size={14} strokeWidth={1.5} /> Télécharger PDF
                </button>
              </div>
            </div>
          </div>
